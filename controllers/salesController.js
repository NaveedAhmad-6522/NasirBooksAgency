    ...
    connection.beginTransaction((err) => {
      ...
      getCustomerId((finalCustomerId) => {
        ...
        const proceedWithSale = (finalCustomerId, isWalkIn) => {
              ...
              (invErr, invRows) => {
                ...
                connection.query(
                  ...
                  (err, saleResult) => {
                    ...
                    return continueSaleProcess(saleResult.insertId, nextInvoice);
                  }
                );
              }
            );
          ...
          connection.query(
           ...
           LIMIT 1`,
            ...
            (findErr, existingSales) => {
              ...
              if (findErr) {
                console.error("SALE LOOKUP ERROR:", findErr);     
                           return connection.rollback(() => {
                  connection.release();
              ...
              if (existingSales.length > 0) {
                ...
                existingSaleTotal = Number(existingSale.total_amount || 0);
                existingSalePaid = Number(existingSale.paid_amount || 0);

                const updatedTotal =
                  existingSaleTotal + total;

                const updatedPaid =
                  existingSalePaid + paidValue;
                ...
                return connection.query(
                 ...
                 WHERE id = ?`,
                  ...
                  (updateErr) => {
                    if (updateErr) {
                      console.error("SALE MERGE ERROR:", updateErr);
                      return connection.rollback(() => {
                        connection.release();
          ...
          function continueSaleProcess(saleId, generatedInvoiceNumber = null) {
            ...
            const saveItems = (index = 0) => {
              ...
              connection.query(
                ...
                (metaErr, metaRows) => {
                  ...
                  if (metaErr) {
                    console.error("BOOK SNAPSHOT ERROR:", metaErr);
            ...
            const updateStock = (index = 0) => {
              ...
              connection.query(
                ...
                (stockErr, stockRows) => {
                  ...
                  if (stockErr) {
                    console.error("STOCK LOCK ERROR:", stockErr);
                    return connection.rollback(() => {
                      connection.release();
              ...
              connection.query(
                ...
                (err, result) => {
                  ...
                  connection.query(
                    ...
                    [saleId],
                    (snapErr, saleRows) => {
                      if (snapErr || !saleRows.length) {
                        return connection.rollback(() => {
                          connection.release();
                          return res.status(500).json({ error: "Sale snapshot fetch failed" });
                        });
                      }
